...
export interface RandomCurveParams {
  harmonicsX: { amp: number; freq: number; phase: number }[];
  harmonicsY: { amp: number; freq: number; phase: number }[];
}

export function randomCurvePoint(params: RandomCurveParams): PointFunction {
  return (p) => {
    const t = p * Math.PI * 2;
    let x = 0, y = 0;
    for (const h of params.harmonicsX) {
      x += h.amp * Math.sin(h.freq * t + h.phase);
    }
    for (const h of params.harmonicsY) {
      y += h.amp * Math.sin(h.freq * t + h.phase);
    }
    return { x: 50 + x, y: 50 + y };
  };
}

export function generateRandomParams(initialSeed?: number): RandomCurveParams {
  let s = initialSeed ?? 0;
  const rng = initialSeed !== undefined
    ? () => { s = (s * 16807 + 0) % 2147483647; return s / 2147483647; }
    : Math.random;

  const count = 2 + Math.floor(rng() * 4); // 2-5 harmonics
  const harmonicsX = Array.from({ length: count }, () => ({
    amp: 5 + rng() * 20,
    freq: 1 + Math.floor(rng() * 7),
    phase: rng() * Math.PI * 2,
  }));
  const harmonicsY = Array.from({ length: count }, () => ({
    amp: 5 + rng() * 20,
    freq: 1 + Math.floor(rng() * 7),
    phase: rng() * Math.PI * 2,
  }));
  return { harmonicsX, harmonicsY };
}
